// //src/pvm/pipelines.rs
// use std::collections::VecDeque;
// use crate::pvm::branch_predictor::{BranchPrediction, BranchPredictor, BranchTargetBuffer, PredictorType};
// use crate::pvm::cache_configs::{CacheSystem, WritePolicy};
...
// use crate::pvm::stacks::ReturnAddressStack;
//
...
//
//     pub branch_target_buffer:BranchTargetBuffer,
...
//     pub hazards: usize,
//
//     pub store_load_hazards: usize,
//     pub load_use_hazards: usize,
//     pub data_dependency_hazards: usize,
// }
//
//
// #[derive(Debug)]
...
//             next_pc: 0,
//
//             // Ajout du BTB et du RAS
//             branch_target_buffer: BranchTargetBuffer::new(32),
//             return_address_stack: ReturnAddressStack::new(32),
//
//         }
//     }
...
//         self.cleanup_hazards();
//         // self.clean_resolved_hazards();
...
//     //     self.metrics.total_cycles += 1;
//     //
//     //     // Vérifier branchement (copier les valeurs nécessaires avant d'emprunter self)
//     //     let branch_info = if let Some(branch) = self.execute_state.get_branch() {
//     //         Some((branch.clone(), true))  // Clone l'instruction de branchement
//     //     } else {
//     //         None
//     //     };
//     //
//     //     // Traiter le branchement si nécessaire
//     //     if let Some((branch, _)) = branch_info {
//     //         let result = self.execute_instruction(&branch, registers)?;
//     //         if result.branch_taken {
//     //             // Si branchement pris, flush le pipeline
//     //             self.flush_pipeline();
//     //             if let Some(target) = result.target_address {
//     //                 self.pc = target;
...
//     //     }
//     //
//     //     // Sauvegarder les états pour les stalls
//     //     let prev_fetch = self.fetch_state.clone();
//     //     let prev_decode = self.decode_state.clone();
//     //     let mut stall_pipeline = false;
//     //
//     //     // Vérifier les hazards dans l'étage decode
//     //     if let Some(instr) = &self.decode_state.instruction {
//     //         match self.hazard_unit.check_hazards(instr, registers) {
//     //             HazardResult::StoreLoad | HazardResult::LoadUse | HazardResult::DataDependency => {
//     //                 stall_pipeline = true;
//     //                 self.stats.stalls += 1;
//     //                 self.stats.hazards += 1;
//     //                 println!("Pipeline Stalled due to hazard");
//     //             }
//     //             _ => {}
//     //         }
//     //     }
//     //
//     //     if !stall_pipeline {
//     //         // Exécuter les étages normalement
//     //         self.writeback_stage(registers)?;
//     //         self.memory_stage(memory)?;
//     //         self.execute_stage(registers)?;
//     //         self.decode_stage(registers)?;
//     //         self.fetch_stage()?;
//     //     } else {
//     //         // En cas de stall, permettre uniquement aux étages plus avancés de continuer
//     //         self.writeback_stage(registers)?;
//     //         self.memory_stage(memory)?;
//     //
//     //         // Restaurer les états pour le stall
//     //         self.fetch_state = prev_fetch;
//     //         self.decode_state = prev_decode;
//     //     }
//     //
...
//
//
//     fn cleanup_hazards(&mut self) {
//         // Nettoyer les hazards après chaque étage
//         if self.writeback_state.result.is_some() {
//             self.hazard_unit.clear_store_address();
//         }
//         if self.memory_state.result.is_some() {
//             self.hazard_unit.clear_load_target();
//         }
//     }
//
//
//
//     /// methode pour mettre à jour les métriques
//     fn update_pipeline_metrics(&mut self) {
//         // Update stage métriques
//         self.metrics.fetch_metrics.update(self.fetch_state.instruction.is_some());
//         self.metrics.decode_metrics.update(self.decode_state.instruction.is_some());
//         self.metrics.execute_metrics.update(self.execute_state.decoded.is_some());
//         self.metrics.memory_metrics_stage.update(self.memory_state.result.is_some());
//         self.metrics.writeback_metrics.update(self.writeback_state.result.is_some());
//
//         // Update hazard métriques
//         self.metrics.hazard_metrics.update(&self.stats);
//     }
//
//
...
//         if let Some(instruction) = self.fetch_buffer.peek() {
//             if instruction.is_branch() {
//                 let prediction = self.branch_predictor.predict(self.pc);
//                 let target = self.branch_target_buffer.get_target(self.pc);
//
//                 if instruction.is_return() {
//                     // Utiliser le RAS pour les retours
//                     if let Some(return_addr) = self.return_address_stack.pop() {
//                         self.next_pc = return_addr;
//                     }
//                 } else if matches!(prediction, BranchPrediction::Taken) && target.is_some() {
//                     self.next_pc = target.unwrap();
//                 }
//             }
//         }
//
//         // Mise à jour du PC et chargement instruction
//         self.pc = self.next_pc;
//         self.next_pc = self.pc.wrapping_add(4); // Incrémente de 4 par défaut
//
...
//         if let Some(instruction) = self.fetch_buffer.pop_front() {
...
//     //                     // Commit le store selon la politique d'écriture
//     //                     let value = result.value as u64; // Conversion explicite de i64 en u64
//     //
//     //                     match self.cache_system.write_policy() {
//     //                         WritePolicy::WriteThrough => {
//     //                             // Écrire immédiatement en mémoire
//     //                             memory.write(addr.0, value)?;
//     //                             self.hazard_unit.commit_store(*addr, result.value);
//     //                         },
//     //                         WritePolicy::WriteBack => {
//     //                             // Juste mettre à jour le cache/store buffer
//     //                             self.cache_system.write(addr.0, value)?;
//     //                             self.hazard_unit.commit_store(*addr, result.value);
//     //                         }
//     //                     }
//     //                 }
...
//                     let value = memory.read(addr.0)?;
//                     self.memory_state.result = Some(ExecutionResult {
//                         value: value as i64, // Conversion explicite de u64 en i64
//                         flags: StatusFlags::default(),
//                         branch_taken: false,
//                         target_address: None,
//                     });
...
//                 // Utiliser les flags plutôt que la valeur du registre
//                 let flags = registers.get_status_flags();
//                 // JumpIf devrait être pris quand les valeurs sont égales (flag zero est true)
//                 let taken = flags.zero;
//
//                 Ok(ExecutionResult {
//                     value: 0,
//                     flags: StatusFlags::default(),
//                     branch_taken: taken,
//                     target_address: if taken { Some(addr.0) } else { None },
//                 })
//             },
//
//
