...
const PunkVMDashboard = () => {
  ...
  const executionTimeline = [
    { cycle: 1, fetch: 1, decode: 0, execute: 0, memory: 0, writeback: 0, event: "MOV R0=10" },
    { cycle: 2, fetch: 1, decode: 1, execute: 0, memory: 0, writeback: 0, event: "MOV R1=20" },
    { cycle: 3, fetch: 1, decode: 1, execute: 1, memory: 0, writeback: 0, event: "MOV R2=10" },
    { cycle: 4, fetch: 1, decode: 1, execute: 1, memory: 1, writeback: 0, event: "MOV R3=5" },
    { cycle: 5, fetch: 1, decode: 1, execute: 1, memory: 1, writeback: 1, event: "MOV R4=0" },
    { cycle: 6, fetch: 1, decode: 1, execute: 1, memory: 1, writeback: 1, event: "MOV R8=0" },
    { cycle: 7, fetch: 1, decode: 1, execute: 1, memory: 1, writeback: 1, event: "MOV R9=0" },
    { cycle: 8, fetch: 1, decode: 1, execute: 1, memory: 1, writeback: 1, event: "JMP +6" },
    { cycle: 9, fetch: 0, decode: 0, execute: 1, memory: 1, writeback: 1, event: "Branch Stall" },
    { cycle: 10, fetch: 1, decode: 0, execute: 0, memory: 1, writeback: 1, event: "MOV R10=1" },
    { cycle: 11, fetch: 1, decode: 1, execute: 0, memory: 0, writeback: 1, event: "CMP R0,R2" },
    { cycle: 12, fetch: 1, decode: 1, execute: 1, memory: 0, writeback: 0, event: "JmpIfEqual" },
    { cycle: 13, fetch: 0, decode: 0, execute: 1, memory: 1, writeback: 0, event: "Branch Stall" },
    { cycle: 14, fetch: 1, decode: 0, execute: 0, memory: 1, writeback: 1, event: "MOV R11=2" },
    { cycle: 15, fetch: 1, decode: 1, execute: 0, memory: 0, writeback: 1, event: "CMP R0,R1" },
    { cycle: 16, fetch: 1, decode: 1, execute: 1, memory: 0, writeback: 0, event: "JmpIfEqual" },
    { cycle: 17, fetch: 0, decode: 0, execute: 1, memory: 1, writeback: 0, event: "Error 0x6E" }
  ];
    ...
    <div className="space-y-6 p-6 bg-gray-50">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="text-right">
            <div className="text-sm text-gray-500">Branch Pred.</div>
            <div className="text-2xl font-bold text-orange-600">{liveMetrics.branchPredictionRate}%</div>
          </div>
        </div>
      </div>
              ...
              <div className="mt-4 grid grid-cols-6 gap-1 text-xs">
                {executionTimeline.map(event => (
                  <div key={event.cycle} className="text-center">
                    ...
                    <div className="text-gray-500 truncate">{event.event}</div>
                  </div>
                ))}
              </div>
            </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    <span>Stalls</span>
                    <span className="font-bold text-red-600">{liveMetrics.stalls} (17.65%)</span>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    <span>Précision</span>
                    <span className="font-bold text-orange-600">{liveMetrics.branchPredictionRate}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span>BTB</span>
              ...
              <CardContent>
                <div className="space-y-2 text-sm">
                  ...
                  <div className="bg-red-50 p-3 rounded">
                    <div className="font-medium">❌ Section 3: Premier Test Conditionnel</div>
                    <div className="text-red-600">Erreur à l'adresse 0x6E après JmpIfEqual</div>
                  </div>
                  <div className="bg-red-50 p-3 rounded">
                    <div className="font-medium">⏳ Section 4-13: Tests Suivants</div>
                    <div className="text-gray-600">Non exécutés à cause de l'erreur</div>
                  </div>
                </div>
              </CardContent>
            </Card>
      ...
      {activeTab === 'issues' && (
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Analyse de l'Issue Critique: PC Incorrect Après Branchement</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  ...
                  <div className="mt-2 text-sm text-red-700">
                    <p>• JmpIfEqual non pris devrait continuer à PC = 0x66</p>
                    <p>• Mais le pipeline essaie d'accéder à l'adresse 0x6E</p>
                    <p>• Décalage de +8 bytes suggests un problème de calcul PC après branchement</p>
                    <p>• Logs montrent: "Branchement non pris, avancement à PC = 0x0000006E"</p>
                  </div>
                </div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <div className="mt-2 text-sm text-blue-700">
                    <p>1. Vérifier le calcul du PC après branchement non pris dans execute.rs</p>
                    <p>2. Corriger la gestion de next_pc dans le pipeline quand branch_taken = false</p>
                    <p>3. Ajouter des logs pour tracer les valeurs PC à chaque étape</p>
                    <p>4. Valider que PC = instruction_addr + instruction_size pour branchements non pris</p>
                  </div>
                </div>

                <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                  <h4 className="font-medium text-gray-900">Analyse des Logs</h4>
                  <pre className="mt-2 text-sm text-gray-700 overflow-x-auto">
{`PC = 0x5E: JmpIfEqual avec target=0x6C
Branch taken = false (ZF=false)
Log: "Branchement non pris, avancement à PC = 0x0000006E"
Expected: PC devrait être 0x5E + 8 = 0x66
Actual: PC devient 0x6E (+8 bytes de trop)

=> Bug dans la gestion de next_pc pour branchements non pris`}
                  </pre>
                </div>
              </div>
            </CardContent>
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div className="bg-blue-600 h-2 rounded-full" style={{width: '73.3%'}}></div>
                  </div>
                  <div className="text-sm text-gray-500">Actuel: 0.88 (73.3%)</div>
                </div>
              ...
              <CardContent>
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold">0.88 IPC</span>
                  </div>
                  <div className="flex justify-between">
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    <tr>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">2</td>
                      <td className="px-6 py-4 whitespace-nowrap">11.76%</td>
                      <td className="px-6 py-4 whitespace-nowrap">Améliorer prédicteur dynamique</td>
                    </tr>
                    <tr>
                      <td className="px-6 py-4 whitespace-nowrap">Control Hazards</td>
                      <td className="px-6 py-4 whitespace-nowrap">3</td>
                      <td className="px-6 py-4 whitespace-nowrap">3</td>
                      <td className="px-6 py-4 whitespace-nowrap">17.65%</td>
                      <td className="px-6 py-4 whitespace-nowrap">Améliorer prédiction et reduce flush</td>
                    </tr>
                    <tr>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">0</td>
                      <td className="px-6 py-4 whitespace-nowrap">0%</td>
                      <td className="px-6 py-4 whitespace-nowrap">Forwarding efficace</td>
                    </tr>
                    <tr>
